<div class="item w-full p-2 border border-solid border-gray-100 flex flex-col transition ease-in-out duration-200 hover:border-purple-900 hover:shadow-2xl">
    <a href="{{ route('shushop.product', [$shoe->category, $shoe->id]) }}" class="h-52 w-full relative">
        @if ($shoe->tag == 'Best Seller')
            <svg class="w-8 absolute -top-6 -left-1 h-20" title="Best Seller" version="1.1" viewBox="0 0 95.05 113.63">
                <defs id="defs2">
        ...
        @endif
        <img loading="lazy" src="{{ asset('./storage/Products/Shoes/' . $shoe->category . '/' . $shoe->thumbnail_main) }}" class="h-full w-full m-0 object-cover">
    </a>
